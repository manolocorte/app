...
list_drugs_1st_lvl = ['1', '2', '3']
list_drug_regulator_choices = [('EMA', 'EMA (European Union)'), ('FDA', 'FDA (United States)'), ('AEMPS', 'AEMPS (Spain)')]
...
def synthetic_phr_form_generator(current_app):
    class CreateSyntheticPHRForm(FlaskForm):
        # Get list of drug levels
        drugs_top_level = Domains.drugs_atc_top_levels()
        drugs_level_2 = Domains.drugs_atc_second_levels()
        list_drugs_top_level = [(None, None)] + [(drug[0], drug[1]) for drug in drugs_top_level]
        list_drugs_level_2 = [(None, None)] + [(drug[0], drug[1]) for drug in drugs_level_2]
        ...
        drug_regulator = SelectField('Drug Regulator:', validators=[DataRequired()], choices=list_drug_regulator_choices)
        drug_random = SelectField('Drugs Randomization:', validators=[DataRequired()], choices=[('Totally Random', 'Totally Random'), ('ATC-Specific', 'ATC-Specific')])
        drug_1st_level = SelectField('1st Lvl:', validators=[], choices=list_drugs_top_level)
        drug_2nd_level = SelectField('2nd Lvl:', validators=[], choices=list_drugs_level_2)
        device_regulator = SelectField('Device Regulator:', validators=[DataRequired()], choices=[('FDA', 'FDA')])
            ...
            def validate_output_file_name(self, filed):
                ...
                for character in filed.data:
                    if character in current_app.config['CT_SPECIAL_CHARACTERS_TO_AVOID']:
                        raise ValidationError('Please do not use special characters in the file name (%s)' % current_app.config['CT_SPECIAL_CHARACTERS_TO_AVOID_STRING'])
    return CreateSyntheticPHRForm()